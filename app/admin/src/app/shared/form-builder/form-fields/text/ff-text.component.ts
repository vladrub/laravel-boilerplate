...
@Component({
    ...
    template: `
        <div [formGroup]="group" class="row" [ngClass]="{'has-error': (!group.controls[settings.name].valid), 'has-success': (group.controls[settings.name].valid)}">
            ...
            <div class="col-sm-9">
                <input class="form-control" [id]="settings.name" [placeholder]="settings.placeholder" [formControlName]="controlName" (keyup)="onChange($event)" type="text">
            </div>
        </div>
    `
})

export class FFTextComponent implements OnInit {
    @Input() settings: any;
